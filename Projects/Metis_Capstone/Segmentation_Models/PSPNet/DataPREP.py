...
class MultiViewSplit(Dataset):
    ...
    def pad_to_divisible(self, array, target_width=160):
        current_width = array.shape[1]
        ...
        if array.ndim == 2:
            padded = np.pad(array, ((0, 0), (pad_left, pad_right)), mode='constant', constant_values=0)
        else:  # Image (H, W, C)
            padded = np.pad(array, ((0, 0), (pad_left, pad_right), (0, 0)), mode='constant', constant_values=0)
    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.mode == "train":
                ...
                if os.path.exists(seg_path):
                    ...
                    if orientation in ['coronal', 'sagittal']:
                        multi_modal_slice = self.pad_to_divisible(multi_modal_slice, target_width=160)
                        seg_slice = self.pad_to_divisible(seg_slice, target_width=160)  # This returns padded array
            ...
            dummy_slice = torch.zeros((240, 160, 4), dtype=torch.float32)
            if self.mode == "train":
                dummy_seg = torch.zeros((240, 160), dtype=torch.long)
