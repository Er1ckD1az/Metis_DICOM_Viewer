...
class PSPNetTrainer:
    ...
    def validate(self, val_loader):
        ...
        all_tp = all_tp.sum(dim=0)
        all_fp = all_fp.sum(dim=0)
        all_fn = all_fn.sum(dim=0)
        all_tn = all_tn.sum(dim=0)
    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            print(f"  Background:      {dice_per_class[0].item():.4f}")
            print(f"  NCR/NET:         {dice_per_class[1].item():.4f}")
            print(f"  Edema:           {dice_per_class[2].item():.4f}")
            print(f"  Enhancing:       {dice_per_class[3].item():.4f}")
