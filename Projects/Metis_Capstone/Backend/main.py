...
from inference import MultiViewEnsemble
import torch
import numpy as np
...
async def run_detection(file: UploadFile = File(...)):
    """Run segmentation detection on uploaded MRI file"""
    if not file.filename.lower().endswith(('.nii', '.nii.gz')):
        raise HTTPException(status_code=400, detail="Only .nii or .nii.gz files supported")
    ...
    try:
        ...
        model_dir = base_dir / "ModelPaths"

        ensemble = MultiViewEnsemble(
            model_dir / "axial_model_best.pth",
            model_dir / "coronal_model_best.pth",
            model_dir / "sagittal_model_best.pth",
        ...
        result = ensemble.predict_volume(volume_dict)

        # Convert to PNG slice-by-slice for frontend display
        slice_images = []
        for z in range(result.shape[2]):
            slice_img = Image.fromarray((result[:, :, z] * 85).astype(np.uint8))  # 0–3 classes
            buffer = io.BytesIO()
            slice_img.save(buffer, format="PNG")
            slice_images.append(base64.b64encode(buffer.getvalue()).decode('utf-8'))

        return {"slices": slice_images}
    finally:
        os.unlink(tmp_path)
